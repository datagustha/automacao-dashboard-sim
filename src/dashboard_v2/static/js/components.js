...
function criarGraficoFase(dados) {
    if (!dados || dados.length === 0) {
        return `
            <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;color:#a0aec0;font-size:14px;">
                <i class="fas fa-chart-pie" style="font-size:40px;opacity:0.3;margin-bottom:10px;"></i>
                <span>Sem dados para o período</span>
            </div>
        `;
    }
    ...
    setTimeout(() => {
        ...
        const labels = dados.map(d => d.fase || 'Outros');
        const valores = dados.map(d => d.total || 0);
        ...
        const options = {
            ...
            colors: ['#7e3d97', '#9b5bad', '#c084d0', '#d8b4e0', '#e9d8fd', '#f3e8ff', '#faf5ff'],
            labels: labels,
            series: valores,
            dataLabels: { enabled: false },
            legend: {
                        ...
                        labels: {
                            ...
                            total: {
                                ...
                                formatter: function(w) {
                                    ...
                                    return 'R$ ' + sum.toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
                                }
                            }
                        }
                    }
