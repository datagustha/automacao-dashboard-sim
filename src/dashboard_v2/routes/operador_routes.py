...
from src.dashboard_v2.services.operador_service import montar_dashboard_operador, montar_performance_operador
...
def api_operador(login):
    ...
    if operador_sessao.get('perfil') != 'adm' and session.get('login') != login:
...
def api_operadores():
    ...
    if operador_sessao.get('perfil') != 'adm':
        return jsonify({'success': False, 'message': 'Acesso negado'}), 403
