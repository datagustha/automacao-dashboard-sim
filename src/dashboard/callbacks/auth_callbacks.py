...
def register_callbacks(app):
    ...
    def render_page(pathname, login_dados):
        ...
        print(f"[DEBUG] login_dados: {login_dados}")
        print("=" * 50)
        ...
        if pathname == '/dashboard':
            if login_dados and 'nome' in login_dados:
                return get_dashboard_layout(login_dados['nome'], login_dados.get("imagem"))
        ...
        # ============================================================
            ...
            # Tenta usar login_dados primeiro
            if login_dados and 'nome' in login_dados:
                return get_pagamentos_layout(login_dados['nome'], login_dados.get('imagem'))
            
            # Se não tem login_dados, tenta recuperar do arquivo
                ...
                try:
                    with open(ULTIMO_LOGIN_FILE, 'r') as f:
                        ...
                        if ultimo_login:
                            ...
                            if operador:
                                return get_pagamentos_layout(operador['nome'], operador.get('imagem'))
                except:
                    pass
    ...
    def gerenciar_autenticacao(n_clicks_login, n_clicks_esqueci, 
        ...
        step = step_store.get('step', 'login') if step_store else 'login'
        ...
        if step == 'validar_senha':
            ...
            with Session(engine) as session:
                ...
                if verificar_senha(user.senha_hash, senha):
                    dados_usuario = {'nome': operador['nome'], 'login': operador['login'], 'imagem': operador.get('imagem')}
        ...
        # VALIDAR TOKEN
        # ============================================================
        if step in ['validar_token_primeiro', 'validar_token_reset']:
            if not token:
    ...
    def fazer_logout(n_clicks):
        ...
        if n_clicks:
            # Remove o arquivo de último login
            try:
                if os.path.exists(ULTIMO_LOGIN_FILE):
